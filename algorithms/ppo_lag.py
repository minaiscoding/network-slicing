...
# -*- coding: utf-8 -*-
"""
PPOLag (PPO-Lagrangian) algorithm for RAN slicing using OmniSafe.

Same cycling curriculum env as PPO, but uses the Lagrangian relaxation
variant that dynamically adjusts the cost penalty multiplier.

Classes:
    RanSlicePPOLagEnv  — OmniSafe CMDP environment
    TrainerPPOLag      — Training orchestrator
"""
...
DEFAULT_RESULTS_PATH       = './pipelines/ppo_lag/'

ENV_ID = "RanSlicePPOLagPipeline-v0"
...
class RanSlicePPOLagEnv(CMDP):
    """Cycling-curriculum PPOLag environment for RAN slicing."""
...
class TrainerPPOLag:
    """Train PPOLag agent with cycling curriculum learning."""
    ...
    def train(self, run_id: int):
        ...
        print(f'\n{"="*60}')
        print(f'=== PPOLag Training Run {run_id} ===')
        print(f'{"="*60}')
