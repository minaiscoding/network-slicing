...
Uses ReportWrapper and saves eval_history_{scenario}.npz inside pipelines/cpo/.

Usage:
    python pipelines/cpo/evaluate.py --checkpoint <path-to-.pt>
"""
...
import algorithms.cpo as cpo_mod
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate CPO checkpoint")
    ...
    cpo_mod._penalty     = args.penalty
    cpo_mod._total_steps = total_steps
    cpo_mod._rng         = np.random.default_rng(args.seed)

    custom_cfgs = {
        "train_cfgs": {"total_steps": max(total_steps, 1), "device": args.device},
        "algo_cfgs":  {
            "steps_per_epoch": max(args.steps_per_epoch, 1),
            "cost_limit": 100.0, "use_cost": True,
    ...
    agent = omnisafe.Agent(algo="CPO", env_id=cpo_mod.ENV_ID, custom_cfgs=custom_cfgs)
