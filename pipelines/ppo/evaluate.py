...
# -*- coding: utf-8 -*-
"""
Evaluate a trained PPO checkpoint across low / medium / congested scenarios.

Uses ReportWrapper and saves eval_history_{scenario}.npz inside pipelines/ppo/.

Usage:
    python pipelines/ppo/evaluate.py --checkpoint <path-to-.pt>
    python pipelines/ppo/evaluate.py --checkpoint runs/.../epoch-10.pt --epochs 5
"""
...
import algorithms.ppo as ppo_mod
...
def main():
    ...
    ppo_mod._penalty     = args.penalty
    ppo_mod._total_steps = total_steps
    ppo_mod._rng         = np.random.default_rng(args.seed)

    custom_cfgs = {
        "train_cfgs": {"total_steps": max(total_steps, 1), "device": args.device},
        "algo_cfgs":  {"steps_per_epoch": max(args.steps_per_epoch, 1)},
        "logger_cfgs": {"use_wandb": False, "save_model_freq": 1},
    }

    print("Initializing PPO agent from checkpoint...")
    agent = omnisafe.Agent(algo="PPO", env_id=ppo_mod.ENV_ID, custom_cfgs=custom_cfgs)
